...
function exportXML() {
    ...
    if (exportSelection) {
		if (confirm("Only selection will be exported.")) {
			for (let i = 0; i < es.length; i++) {
				if (es[i].selected) {
					newstr += compi_obj(i);
				}
...
function patchBoxSizes() {
	bo_x["vehicle_veh_hh"] = -15;
	bo_y["vehicle_veh_hh"] = -15;
	bo_w["vehicle_veh_hh"] = 30;
	bo_h["vehicle_veh_hh"] = 30;
}
...
function updateClipboardDiv() {
	if (!localStorage.clipboardItems) {
		localStorage.clipboardItems = '[{"name": "Item", "url": ""}]';
	}
